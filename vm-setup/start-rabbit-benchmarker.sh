...
gcloud compute ssh $GCLOUD_AUTH -- "docker run -it -e RABBIT_HOST=$RABBIT_HOST -e METRICS_HOST=$METRICS_HOST -e IS_CLUSTERING=1 -e DRIVER_TYPE=rabbit ihsw/rabbit-benchmarker ./bin/run-app rabbit-benchmarker -d 1m -w 1"
