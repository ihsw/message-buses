...
gcloud compute ssh $GCLOUD_AUTH -- "docker run -it -e NATS_HOST=$NATS_HOST -e METRICS_HOST=$METRICS_HOST -e IS_CLUSTERING=1 --entrypoint=\"\" ihsw/nats-benchmarker ./bin/run-app nats-benchmarker -d 2m -w 50"
