...
gcloud compute ssh $DEFAULT_USER@nats-consumer-very-high-cpu -- $'docker start $(docker ps -a | grep -i telegraf-collector | awk \'{print $1}\')'
...
gcloud compute ssh $DEFAULT_USER@nats-producer-very-high-cpu -- $'docker start $(docker ps -a | grep -i telegraf-collector | awk \'{print $1}\')'
